...
export class FlightsComponent implements OnInit {
  ...
  generateFreePlaces(seats: ISeats): (string | boolean)[][] {
    ...
    const availablePlaces: (string|boolean)[][] = new Array(rowCount).fill(false);
    const availablePlacesFlatArray: string[] = [];
    const isIncompleteRow: boolean = seats.total / PLACES_PER_ROW > Math.floor(seats.total / PLACES_PER_ROW);
    const incompleteRowLength: number = seats.total - Math.floor(seats.total / PLACES_PER_ROW) * PLACES_PER_ROW;
    let ind: number = 0;

    availablePlaces.forEach((x, ind, arr) => arr[ind] = new Array(PLACES_PER_ROW).fill(false));
    ...
    while (ind < seats.avaible) {
      ...
      const newPlaceLetterInd: number = newPlaceInd - Math.floor(newPlaceInd / PLACES_PER_ROW) * PLACES_PER_ROW;
      
      availablePlaces[newPlaceRowInd][newPlaceLetterInd] = availablePlacesFlatArray[newPlaceInd];
