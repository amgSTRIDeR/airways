...
export class DateOnTicketComponent implements OnChanges {
  ...
  ngOnChanges(): void {
    this.flightDate = this.isDateFrom? this.flightInfo?.takeoffDate : this.flightInfo?.landingDate;
    this.flightGmt = this.isDateFrom? this.flightInfo?.form.gmt : this.flightInfo?.to.gmt;
    this.flightCity = this.isDateFrom ? this.flightInfo?.form.city : this.flightInfo?.to.city;
  }
}
